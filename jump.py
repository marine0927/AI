...
def pull_screenshot():#将手机屏幕截图并且传至电脑中
    os.system('adb shell /system/bin/screencap -p /sdcard/screenshot.png')
    os.system('adb pull /sdcard/screenshot.png h:/Jump/screenshot.png')
...
def check_screenshot():
    ...
    os.system('adb shell /system/bin/screencap -p /sdcard/screenshot.png')
    os.system('adb pull /sdcard/screenshot.png h:/Jump/screenshot.png')
    Image.open('h:/Jump/screenshot.png').load()


def main():
    check_screenshot()

    i, next_rest, next_rest_time = (0, random.randrange(3, 10),#随机生成休息间隔
    ...
    while True:
        ...
        ts = int(time.time())
        print(ts, piece_x, piece_y, board_x, board_y)#输出位置坐标
        set_button_position(im)
        jump(math.sqrt((board_x - piece_x) ** 2 + (board_y - piece_y) ** 2))#求距离并且跳跃
        im.close()
        i += 1
        if i == next_rest:
